    ...
    <div className="relative flex">
      <div className="flex flex-col w-full">
        ...
        <div className="grid grid-cols-1 gap-2 mt-5 md:grid-cols-2 lg:grid-cols-3">
          {books.map((book, i) => {
            const link = new URL(`https://www.google.com/search`);
            link.searchParams.append("q", `${book.name} by ${book.author}`);

            return (
              <a key={i} href={link.toString()} target="_blank" className="flex flex-col p-6 rounded bg-white/5">
