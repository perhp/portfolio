...
export interface LandingTemplateProps {
  page: LandingPage;
  /** Route prefix, e.g. "/services" or "/industries". */
  basePath: string;
  /** Breadcrumb label for the index, e.g. "Services". */
  indexLabel: string;
  proof: WorkItem[];
  related: RelatedLink[];
  /** Optional second list of cross links, e.g. services from an industry page. */
  crossLinks?: { title: string; items: RelatedLink[] };
  /** Copy overrides for sections whose wording differs between page families. */
  copy?: Partial<{
    fitTitle: string;
    fitIntro: string;
...
/** Shared layout for SEO landing pages (services, industries). JSON-LD is emitted here so every family gets it. */
export function LandingTemplate({ page, basePath, indexLabel, proof, related, crossLinks, copy = {} }: LandingTemplateProps) {
  const pageUrl = `${site.url}${basePath}/${page.slug}`;
  const subject = `${page.intent}: project inquiry`;
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: indexLabel, item: `${site.url}${basePath}` },
  ...
  return (
    <>
      ...
      <Section
        ...
        title={copy.fitTitle ?? "An honest fit check before we talk"}
        intro={copy.fitIntro ?? "I would rather tell you no on the first call than waste your budget. Here is what a good match looks like for this kind of work."}
      >
      ...
      {/* Deliverables */}
      <Section id="what" eyebrow="What I do" title={copy.deliverablesTitle ?? "What the engagement typically covers"}>
      ...
      {/* Proof */}
      <Section id="proof" eyebrow="Proof" title={copy.proofTitle ?? "Clients who kept me around"} intro={page.proofIntro}>
                  ...
                  <h3 className="text-2xl display">
                    {w.link ? (
                      <a href={w.link} target="_blank" rel="noreferrer" className="inline-flex items-baseline gap-1 transition-colors group hover:text-accent">
                        {w.client}
                        <ArrowUpRightIcon className="self-center w-4 h-4 transition-colors text-ink-faint group-hover:text-accent" aria-hidden="true" />
                      </a>
                    ) : (
                      w.client
                    )}
      ...
      {/* Related */}
      <Section id="related" eyebrow="Related" title={copy.relatedTitle ?? "Other ways I can help"}>
        <RelatedList items={related} />
        {crossLinks && (
          <div className="pt-8 mt-12 border-t border-line">
            <p className="eyebrow">{crossLinks.title}</p>
            <div className="mt-6">
              <RelatedList items={crossLinks.items} />
            </div>
          </div>
        )}
        <p className="mt-8 text-sm text-ink-muted">
          <Link href={basePath} className="link-ul">
      ...
      <section id="contact" className="text-white scroll-mt-16 bg-ink">
        <div className="py-20 container-x sm:py-28">
          ...
          <h2 className="mt-4 text-4xl text-white display sm:text-5xl text-balance">Tell me what you are building</h2>
...
function RelatedList({ items }: { items: RelatedLink[] }) {
  return (
    <ul className="grid gap-x-8 gap-y-6 sm:grid-cols-3">
      {items.map((r) => (
        <li key={r.href}>
          <Link href={r.href} className="block pt-4 transition-colors border-t group border-line hover:border-accent">
...
/** Next.js Metadata for a landing page. */
export function landingMetadata(page: LandingPage, basePath: string) {
  const title = `${page.title} | ${site.name}`;
  const url = `${basePath}/${page.slug}`;
  return {
    title,
    description: page.description,
